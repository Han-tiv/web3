# 🎯 AI 交易 Prompt V2.1 - Valuescan方法论（实战信号增强版）

**基于 Valuescan 社群 94,193 条消息实战经验**  
**V2.1 新增**: 基于猎庄一号16,928条真实交易数据分析，新增「真空区检测」和「跌破不收回」信号  
...
**5m级别**:
- **5m回落>5%**: PARTIAL 40-50% (见顶,部分止盈)
- **5m回落>8%**: FULL (快速回落,全平)
- **倒V形态**(3根: 低-高-低): PARTIAL 50% (见顶信号)
...
**真空区风险管理**:
- **持仓进入真空区**: 
  * 盈利>5% → PARTIAL 50% (锁定利润,真空区波动大)
  * 盈利<5% → HOLD但密切关注(可能快速到下一关键位)
  * 亏损ANY → 观察是否跌破不收回,准备止损

**跌破不收回信号**:
- **支撑跌破 + 3根15m未收回**: 
  * 盈利ANY → PARTIAL 70-100% (趋势反转,保护利润)
  * 亏损<-2% → FULL (确认破位,立即止损)
- **1h破位确认**(1-2根K线未收回):
  * 无论盈亏 → FULL (强破位,全平)
...
**部分止盈触发条件（降低）**:
基于实战发现，以下情况立即部分止盈：
- 盈利>8% + 真空区内 → PARTIAL 50%
- 盈利>5% + 跌破不收回初步确认 → PARTIAL 60%
- 盈利>3% + 15m连续3根阴线 + 在真空区 → PARTIAL 40%
...
1. **兼容性**: V2版本与V1版本可以共存,通过函数名区分
2. **评分系统**: 新增valuescan_score评分,≥6分才开仓
3. **关键位优先**: 开仓50%权重在关键位,持仓60%权重在关键位
4. **代码兜底**: 持仓>4h盈利<1%自动平仓,亏损>-5%自动平仓
5. **决策透明**: score_breakdown详细列出各项评分
