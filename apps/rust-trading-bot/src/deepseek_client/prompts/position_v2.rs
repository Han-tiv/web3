use crate::ai::PromptBuilder;
use crate::prompt_contexts::PositionPromptContext;

// 备用持仓管理 prompt,供 DeepSeek V2 使用
#[allow(dead_code)]
pub fn build_position_management_prompt_v2(ctx: &PositionPromptContext<'_>) -> String {
    let symbol = ctx.symbol;
    let side = ctx.side;
    let entry_price = ctx.entry_price;
    let current_price = ctx.current_price;
    let profit_pct = ctx.profit_pct;
    let hold_duration_hours = ctx.hold_duration_hours;
    let klines_5m = ctx.klines_5m;
    let klines_15m = ctx.klines_15m;
    let klines_1h = ctx.klines_1h;
    let indicators = ctx.indicators;
    let support_text = ctx.support_text;
    let deviation_desc = ctx.deviation_desc;
    let current_stop_loss = ctx.current_stop_loss;
    let current_take_profit = ctx.current_take_profit;
    let kline_5m_text = PromptBuilder::format_klines(klines_5m, "5m", 15);
    let kline_15m_text = PromptBuilder::format_klines(klines_15m, "15m", 15);
    let kline_1h_text = PromptBuilder::format_klines(klines_1h, "1h", 20);
    let indicators_text = PromptBuilder::format_indicators(indicators);
    let key_levels_text = PromptBuilder::identify_key_levels(klines_1h, current_price);

    format!(
        r#"你是专业加密货币交易员,采用 Valuescan "关键位止盈法" 管理持仓。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 【持仓状态】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

- 币种: {}
- 方向: {}
- 入场价: ${:.4}
- 当前价: ${:.4}
- 浮动盈亏: {:+.2}%
- 持仓时间: {:.1}小时
- 当前止损: {}
- 当前止盈: {}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📈 【市场环境】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

{}

{}

{}

{}

{}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 【关键位与支撑阻力】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

{}

- 支撑位分析: {}
- 价格偏离度: {}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔥 【实战信号应用】(基于16,928条真实交易数据) ⭐ NEW
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1. 真空区风险管理**:
- **场景**: 价格进入无支撑/阻力的真空区
- **策略**:
  * 盈利中: 快速移动止损到盈亏平衡点或锁定50%利润
  * 亏损中: 严格止损，不要在真空区加仓
  * 突破真空区: 若放量突破，可移动止损跟随

**2. 跌破不收回应对**:
- **场景**: 价格跌破关键支撑且15m/1h未收回
- **策略**:
  * 多单: 立即止损或减仓，不要抱有幻想
  * 空单: 持有，移动止损到被跌破的关键位上方
  * 假跌破: 若快速收回(V反)，可恢复原止损或加仓

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💡 【决策逻辑优先级】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**优先级 1: 关键位止盈 (权重 60%)**
- 价格到达强阻力位(1h/4h级别) -> 减仓50%
- 价格跌破关键支撑(1h收盘确认) -> 清仓/止损
- 距离关键位<2% -> 准备操作

**优先级 2: K线反转信号 (权重 30%)**
- 顶部形态: 射击之星/看跌吞没/黄昏之星 -> 减仓
- 底部形态: 锤子线/看涨吞没/早晨之星 -> 持有/加仓
- 假突破: 突破阻力后快速回落 -> 清仓

**优先级 3: 盈利时间参考 (权重 10%)**
- 盈利>10%且持仓<4h -> 移动止损到入场价+1%
- 盈利>20% -> 移动止损锁定10%利润
- 持仓>24h且盈利<2% -> 考虑换仓(资金效率低)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 【输出格式】严格 JSON 格式响应
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

请以 JSON 对象格式返回持仓管理决策：

{{
    "action": "HOLD|CLOSE|PARTIAL_CLOSE|ADD",
    "confidence": "HIGH|MEDIUM|LOW",
    "quantity_pct": 操作比例(0-100),
    "reason": "决策理由(必含: 关键位状态+K线形态+实战信号, 限100字)",
    "new_stop_loss": 新止损价(可选, 0为不调整),
    "new_take_profit": 新止盈价(可选, 0为不调整),
    "vacuum_zone_analysis": {{
        "in_vacuum": false,
        "risk_level": "LOW|MEDIUM|HIGH",
        "action_suggestion": "真空区操作建议"
    }},
    "break_without_recovery": {{
        "detected": false,
        "level_broken": 0.0,
        "confirmation": "NONE|PARTIAL|CONFIRMED"
    }},
    "risk_assessment": "当前风险评估(如: RSI超买, 逼近阻力位)",
    "next_check_interval": "NORMAL|URGENT"
}}

**操作说明**:
- **HOLD**: 继续持有,可调整止损/止盈
- **CLOSE**: 全部平仓(止盈或止损)
- **PARTIAL_CLOSE**: 减仓(如到达第一目标位)
- **ADD**: 加仓(如回踩支撑确认)

**重要**: 请严格以 JSON 格式输出，不要包含任何额外说明或markdown代码块。

请基于Valuescan关键位止盈法给出决策(JSON格式)!
"#,
        symbol,
        side,
        entry_price,
        current_price,
        profit_pct,
        hold_duration_hours,
        current_stop_loss
            .map(|p| format!("${:.4}", p))
            .unwrap_or_else(|| "未设置".to_string()),
        current_take_profit
            .map(|p| format!("${:.4}", p))
            .unwrap_or_else(|| "未设置".to_string()),
        kline_5m_text,
        kline_15m_text,
        kline_1h_text,
        indicators_text,
        key_levels_text,
        key_levels_text, // 再次使用关键位文本作为"关键位与支撑阻力"部分
        support_text,
        deviation_desc
    )
}
