...
impl ExchangeClient for OkxClient {
    ...
    async fn get_account_info(&self) -> Result<AccountInfo> {
        ...
        let account = resp.data.and_then(|d| d.into_iter().next()).ok_or_else(|| anyhow!("OKX账户数据为空"))?;
        ...
        if let Some(details) = &account.details {
            for detail in details {
                if detail.ccy == "USDT" || detail.ccy == "USDC" {
                    let avail = detail.availBal.as_ref().and_then(|s| s.parse().ok()).unwrap_or(0.0);
                    let frozen = detail.frozenBal.as_ref().and_then(|s| s.parse().ok()).unwrap_or(0.0);
            ...
            available_balance = account.availEq.as_ref()
                .filter(|s| !s.is_empty())
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.0);
            margin_used = account.frozenBal.as_ref().and_then(|s| s.parse().ok()).unwrap_or(0.0);
        }
    ...
    async fn get_symbol_trading_rules(&self, symbol: &str) -> Result<TradingRules> {
        ...
        let okx_symbol = self.format_symbol(symbol);
        let request_path = format!("/api/v5/public/instruments?instType=SWAP&instId={}", okx_symbol);
        ...
        let resp: OkxResponse = serde_json::from_str(&body)?;
        let data = resp.data.and_then(|d| d.into_iter().next()).ok_or_else(|| anyhow!("交易规则数据为空"))?;

        let rules = TradingRules {
            step_size: data["lotSz"].as_str().and_then(|s| s.parse().ok()).unwrap_or(0.001),
            min_qty: data["minSz"].as_str().and_then(|s| s.parse().ok()).unwrap_or(1.0),
    ...
    async fn set_position_mode(&self, dual_side: bool) -> Result<()> {
        ...
        info!("✅ OKX设置持仓模式成功: {}", if dual_side { "双向" } else { "单向" });
        Ok(())
    }
