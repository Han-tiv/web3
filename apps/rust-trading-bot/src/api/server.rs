...
use crate::config::database::{AiAnalysisRecord, Database, TradeRecord as DbTradeRecord};
...
async fn get_telegram_signals(
    State(state): State<Arc<AppState>>,
) -> Json<Vec<crate::config::database::TelegramSignalRecord>> {
...
fn save_telegram_signal(
    ...
    raw_message: &str,
    timestamp: f64,
) -> crate::config::database::DbResult<i64> {
