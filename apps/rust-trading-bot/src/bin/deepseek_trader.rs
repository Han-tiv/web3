...
fn wait_for_next_period(interval_minutes: u64) -> Duration {
    ...
    if display_minutes > 0 {
        info!("🕒 等待 {} 分 {} 秒到整点...", display_minutes, display_seconds);
    } else {
...
async fn calculate_intelligent_position<T: ExchangeClient>(
    ...
    info!("💰 可用USDT余额: {:.2}, 下单基数: {:.2}", usdt_balance, config.base_usdt);
    ...
    // 计算建议投入USDT金额
    let suggested_usdt = config.base_usdt * confidence_multiplier * trend_multiplier * rsi_multiplier;
    ...
    if final_amount > coin_amount {
        info!("   ⚠️  调整到最小交易量: {:.6} {}", final_amount, symbol_name);
    }
    
    info!("🎯 最终仓位: {:.2} USDT → {:.6} {}", final_usdt, final_amount, symbol_name);
...
async fn main() -> Result<()> {
    ...
    info!("   最小交易量: {} {}", config.amount, format!("{:?}", config.trading_symbol));
    ...
    match config.exchange {
        ...
        ExchangeType::Okx => {
            let api_key = std::env::var("OKX_API_KEY")
                .expect("❌ 缺少 OKX_API_KEY 环境变量");
            let secret = std::env::var("OKX_SECRET")
                .expect("❌ 缺少 OKX_SECRET 环境变量");
            let passphrase = std::env::var("OKX_PASSWORD")
                .expect("❌ 缺少 OKX_PASSWORD 环境变量");
        ...
        ExchangeType::Gate => {
            let api_key = std::env::var("GATE_API_KEY")
                .expect("❌ 缺少 GATE_API_KEY 环境变量");
            let secret = std::env::var("GATE_SECRET")
                .expect("❌ 缺少 GATE_SECRET 环境变量");
    ...
    if let Some(pos) = current_position {
        ...
        if is_reverse_signal {
            ...
            if signal.confidence != "HIGH" {
                info!("   当前持仓: {} | 信号: {} | 信心: {}", pos.side, signal.signal, signal.confidence);
            ...
            let same_signal_count = recent_signals.iter()
                .filter(|s| s.signal == signal.signal)
                .count();
            
            if same_signal_count >= 2 {
                info!("   ⚠️  最近3次中已出现{}次{}信号，避免频繁反转", same_signal_count, signal.signal);
                return true;
            }
        }
    }
...
async fn get_klines<T: ExchangeClient>(
    exchange: &Arc<T>,
    symbol: &str,
) -> Result<Vec<Kline>> {
        ...
        "BUY" => {
            match current_position {
                ...
                Some(pos) if pos.side == "long" => {
                    ...
                    if size_diff_abs >= 0.0001 {  // 有显著差异
                        if size_diff > 0.0 {
                            // 加仓
                            let add_size = size_diff;
                            info!("📈 多仓加仓 {:.6} BTC (当前:{:.6} → 目标:{:.6})", 
                            ...
                            info!("📉 多仓减仓 {:.6} BTC (当前:{:.6} → 目标:{:.6})", 
                                  reduce_size, pos.size, position_size);
                            
                            match exchange.close_position(
                                &config.symbol,
                                "long",
                                reduce_size
                            ).await {
                        ...
                        info!("⏸️  多仓仓位合适，保持现状 (当前:{:.6}, 目标:{:.6})", 
                              pos.size, position_size);
                    }
                }
                Some(pos) if pos.side == "short" => {
                    // 有空头持仓，先平空再开多
                    info!("🔄 平空仓 {:.6} BTC 并开多仓 {:.6} BTC", pos.size, position_size);
                    
                    // 平空仓
                    match exchange.close_position(&config.symbol, "short", pos.size).await {
                        Ok(_) => {
                            info!("✅ 平空仓成功");
            ...
            match current_position {
                ...
                Some(pos) if pos.side == "short" => {
                    ...
                    if size_diff_abs >= 0.0001 {
                        ...
                        info!("⏸️  空仓仓位合适，保持现状 (当前:{:.6}, 目标:{:.6})", 
                              pos.size, position_size);
                    }
                }
                Some(pos) if pos.side == "long" => {
                    // 有多头持仓，先平多再开空
                    info!("🔄 平多仓 {:.6} BTC 并开空仓 {:.6} BTC", pos.size, position_size);
                    
                    // 平多仓
                    match exchange.close_position(&config.symbol, "long", pos.size).await {
                        Ok(_) => {
                            info!("✅ 平多仓成功");
                            sleep(Duration::from_secs(1)).await;
