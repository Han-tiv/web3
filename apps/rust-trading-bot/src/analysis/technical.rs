use crate::analysis::market_data::{Kline, TechnicalIndicators};
use log::info;
...
impl TechnicalAnalyzer {
    ...
    pub fn determine_trend(&self, indicators: &TechnicalIndicators, current_price: f64) -> String {
        ...
        let price_above_sma5 = current_price > indicators.sma_5;
        let sma5_above_sma20 = indicators.sma_5 > indicators.sma_20;
        let macd_positive = indicators.macd > indicators.macd_signal;

        if price_above_sma20 && price_above_sma5 && sma5_above_sma20 && macd_positive {
            "强势上涨".to_string()
        } else if price_above_sma20 && sma5_above_sma20 {
            "上涨趋势".to_string()
        } else if !price_above_sma20 && !price_above_sma5 && !sma5_above_sma20 && !macd_positive {
            "强势下跌".to_string()
        } else if !price_above_sma20 && !sma5_above_sma20 {
            "下跌趋势".to_string()
        } else {
