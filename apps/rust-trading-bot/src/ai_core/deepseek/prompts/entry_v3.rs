use crate::ai_core::prompt_builder::PromptBuilder;
use crate::prompt_contexts::EntryPromptContext;

/// V3 Entry Prompt - 交易员思维版
/// 整合: Valuescan关键位 + Fibonacci回撤 + 多周期共振
/// 核心改进: 不追涨杀跌，等回撤到关键位确认反转再入场
pub fn build_entry_analysis_prompt_v3(ctx: &EntryPromptContext<'_>) -> String {
    let symbol = ctx.symbol;
    let alert_type = ctx.alert_type;
    let alert_message = ctx.alert_message;
    let fund_type = ctx.fund_type;
    let flow_text = ctx.flow_text;
    let klines_5m = ctx.klines_5m;
    let klines_15m = ctx.klines_15m;
    let klines_1h = ctx.klines_1h;
    let current_price = ctx.current_price;

    let kline_5m_text = PromptBuilder::format_klines(klines_5m, "5m", 12);
    let kline_15m_text = PromptBuilder::format_klines(klines_15m, "15m", 12);
    let kline_1h_text = PromptBuilder::format_klines(klines_1h, "1h", 15);
    let key_levels_text = PromptBuilder::identify_key_levels(klines_1h, current_price);

    format!(
        r#"你是加密货币交易员。用眼睛看盘，用经验判断，不是执行规则的机器。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 交易哲学
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

顺大势，逆小势。等价格回到关键位，确认反转再动手。
不追涨杀跌，宁可错过不可做错。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 当前数据
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

币种: {} | 当前价: ${:.4}
信号: {} ({})
{}
{}

{}
{}
{}

{}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 三屏看盘 (1h → 15m → 5m)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1h - 定方向**
整体是涨是跌？高低点怎么走？
上涨趋势找回撤做多，下跌趋势找反弹做空。

**15m - 找位置**
价格到关键位了吗？是否在Fib 0.5/0.618回撤区？

**5m - 等信号**
到了位置别急，等反转K线出现：锤子、吞没、Pin Bar，要放量才算数。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔍 关键位识别
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1h K线关键位 (核心)**:
1. 大K线中点 - 1h大阳线/大阴线的实体中间位置
   - 大阳线中点 = 回撤支撑 (多头成本线)
   - 大阴线中点 = 反弹阻力 (空头成本线)
2. 影线聚集区 - 多根K线在同一价位留下上/下影线
3. 前期高低点 - 7-30天内的明显拐点
4. 整数关口 - $1.00, $10.00 心理价位
5. 成交量堆积 - 历史上该价位成交量特别大

**位置判断**:
- 距关键位 < 3%：接近边界，准备动手
- 距关键位 3-10%：安全区，可操作
- 距关键位 > 10%：箱体中间飘，等到边界

**Fib回撤 (辅助验证)**:
- 0.5 和 0.618 是黄金回撤区
- 关键位与Fib重合 = 最佳入场点

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ 真空区警告
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

两个关键位之间的"空白地带"，没有明显支撑阻力：
- 特征：连续多根K线没有影线聚集，成交量稀薄
- 风险：价格容易快速波动，止损容易被扫
- 策略：在真空区边缘等，不要在里面开仓

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💰 资金流向验证
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

技术面看价格，资金面看"谁在买"：
- 做多要看到：资金流入、大单买入、主力加仓
- 做空要看到：资金流出、大单卖出、主力减仓
- 背离要小心：价涨资金出=诱多，价跌资金入=诱空

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ 做多条件
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

趋势向上
+ 价格回撤到支撑位 (Fib 0.5/0.618 附近更佳)
+ 5m出现反转看多K线 + 放量
+ 资金流入配合
+ 不在真空区
→ 入场做多

止损：支撑位下方 或 前低下方 或 Fib 0.618下方 (取近者)
止盈：Fib 0.236 → 前高 → Fib 1.618延伸

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ 做空条件
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

趋势向下
+ 价格反弹到阻力位 (Fib 0.5/0.618 附近更佳)
+ 5m出现反转看空K线 + 放量
+ 资金流出配合
+ 不在真空区
→ 入场做空

止损：阻力位上方 或 前高上方 或 Fib 0.618上方 (取近者)
止盈：Fib 0.236 → 前低 → Fib 1.618延伸

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❌ 不做的情况
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

- 趋势不清，看不懂方向
- 价格在真空区飘着
- 没到关键位，硬猜底顶
- 没有反转K线，硬冲进去
- 资金和价格方向打架 (背离)
- 风险收益比 < 2:1
- 感觉不对劲

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 输出 (严格JSON，无额外说明)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

{{
    "signal": "BUY|SELL|SKIP",
    "confidence": "HIGH|MEDIUM|LOW",
    "entry": 入场价,
    "stop_loss": 止损价,
    "targets": {{"tp1": Fib0.236目标, "tp2": 前高低目标, "tp3": Fib1.618延伸}},
    "rr_ratio": 风险收益比,
    "position_pct": 仓位百分比,
    "key_levels": {{"support": 支撑价, "resistance": 阻力价, "big_candle_mid": 大K线中点价}},
    "fib_levels": {{"fib_0236": 价格, "fib_05": 价格, "fib_0618": 价格, "fib_1618": 价格}},
    "in_vacuum": true/false,
    "trend": "UP|DOWN|RANGE",
    "reversal_candle": "锤子|吞没|PinBar|无",
    "volume_confirm": true/false,
    "fund_flow": "流入|流出|中性|背离",
    "reason": "为什么做这个决定（说人话，80字内）",
    "warnings": ["风险提示"]
}}

现在请分析并给出交易决策!
"#,
        symbol,
        current_price,
        alert_type,
        fund_type,
        alert_message,
        flow_text,
        kline_1h_text,
        kline_15m_text,
        kline_5m_text,
        key_levels_text,
    )
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::ai_core::deepseek::Kline;

    #[test]
    fn test_prompt_v3_generation() {
        let klines: Vec<Kline> = vec![];
        let ctx = EntryPromptContext {
            symbol: "BTCUSDT",
            alert_type: "资金流入",
            alert_message: "BTC大单买入",
            fund_type: "合约",
            flow_text: "24h净流入",
            zone_1h_summary: "",
            zone_15m_summary: "",
            entry_action: "",
            entry_reason: "",
            klines_5m: &klines,
            klines_15m: &klines,
            klines_1h: &klines,
            current_price: 50000.0,
        };

        let prompt = build_entry_analysis_prompt_v3(&ctx);
        assert!(prompt.contains("交易哲学"));
        assert!(prompt.contains("大K线中点"));
        assert!(prompt.contains("真空区"));
    }
}
